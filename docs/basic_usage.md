...
The central use case for AimsPy. Inject a pre-trained DeepH Hamiltonian as the initial guess and converge SCF in a single iteration:
...
| `ADD`     | Accumulate external blocks on top of the live H0 | `source=` |
...
Round-trip from AimsPy to the DeepH on-disk format:
...
If SCF crashes, use `force_close()` (always safe — swallows Fortran errors from any state) and create a fresh `Calculator`:
...
The `aimspy` command provides a single subcommand, `patch`, for managing the bundled FHI-aims patch. Full patching instructions live in [Installation & Setup](./installation_and_setup.md#patching-fhi-aims); the CLI surface is:
...
- **`continue_calc/run.py`** — warmstart demo. Loads the previous run's DeepH output via `DeepHData.from_directory`, applies `Strategy.REPLACE` via `modify_init_ham(source=data)`, and shows SCF converging in a single iteration.
...
2. **New callback** — follow the four-place extension contract (Fortran patch + `callback_types.py` + `prototypes.py` + `registry.py`).
