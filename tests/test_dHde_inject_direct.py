#!/usr/bin/env python
"""Integration test: DFPT dH/de warmstart (direct mode).

Loads DeepH data from deeph_dHde_out/ (produced by test_dHde_capture.py),
registers modify_init_first_order_ham(source=, REPLACE), and runs SCF+CPSCF.
...
Usage:
    ...
    mpiexec -np 8 python tests/test_dHde_inject_direct.py
"""
...
INJECT_LOG = "aims_dHde_inject_direct.out"
...
# =============================================================================
# Load DeepH data (each rank independently)
# =============================================================================
_info("=" * 60)
_info("Direct mode warmstart: modify_init_first_order_ham(source=, REPLACE)")
_info("=" * 60)

dd_warm = DeepHData.from_directory(DEEPH_DIR)
if rank == 0:
    _info(f"  Loaded DeepHData: {dd_warm}")
    if dd_warm.first_order_hamiltonian_entries is None:
        print("ERROR: no first_order_hamiltonian_entries in DeepH data")
        sys.exit(1)


# =============================================================================
# Run SCF + CPSCF with dH/de injection
# =============================================================================
...
try:
    ...
    if rank == 0:
        ...
        _info(f"  CPSCF iterations (direct inject): {n_inject_iters}")
        all_ok &= _ok("CPSCF converged (direct inject)", converged)
        ...
        if n_capture_iters > 0 and n_inject_iters > 0:
            ...
            all_ok &= _ok(
                "direct inject reduces CPSCF iterations",
...
rss_after = _rss_snapshot("after direct warmstart")
...
if rank == 0:
    ...
    if all_ok and mem_ok:
        _info("DHDE INJECT DIRECT TEST PASSED")
    elif all_ok:
        _info("DHDE INJECT DIRECT TEST PASSED (memory check failed)")
    else:
        _info("DHDE INJECT DIRECT TEST FAILED")
    _info("=" * 60)
    if not all_ok:
        sys.exit(1)
