...
Covers:
  ...
  5. capture_initial_hamiltonian (export initial Hamiltonian as AimspyMatrix)
  6. DeepHData from_directory + save
  7. DeepHData from_aimspy (hamiltonian+overlap+initial_hamiltonian export
     with template)
"""
...
LIB_PATH = Path(
    "/home/deeph/software/calc/aimspy/FHI-aims-deeph/build/libaims.250822_1.scalapack.mpi.so"
)
...
config = CalculatorConfig(
    lib_path=LIB_PATH,
    logfile=Path("aims_regression.out"),
...
try:
    ...
    check("structure.orbit_per_atom sum = n_basis", s.orbit_per_atom.sum() == 90)
    ...
    if rank == 0:
        ...
        check(f"aims→aimspy→aims roundtrip max|diff|={max_diff:.2e}", max_diff < 1e-10)

        # ── 5. capture_initial_hamiltonian ───────────────────────────
        check("initial_hamiltonian exists", h_init is not None)
        if h_init is not None:
            check("initial_hamiltonian n_pairs > 0", h_init.n_pairs > 0)

        # ── 6. aimspy → DeepH → aimspy roundtrip ─────────────────────
        deeph_out = DeepHData.from_aimspy(s, hamiltonian=H_aimspy)
        ...
        check(
            "deeph_out n_basis == 90",
            deeph_out.n_basis == 90,
            f"n_basis={deeph_out.n_basis}",
        )

        H_aimspy2 = deeph_out.to_aimspy(s)
        check(
            "deeph roundtrip n_pairs match",
        ...
        check(f"aimspy→deeph→aimspy→aims max|diff|={max_diff2:.2e}", max_diff2 < 1e-6)

        # ── 7. from_aimspy with overlap and initial_hamiltonian ─────
        dd_all = DeepHData.from_aimspy(
            s,
        ...
        check(
            "from_aimspy n_basis == 90",
            dd_all.n_basis == 90,
            f"n_basis={dd_all.n_basis}",
        )
        check("from_aimspy has overlap_entries", dd_all.overlap_entries is not None)
        check(
        ...
        check(
            "save/load full H entries", np.allclose(dd_all.entries, dd_reloaded.entries)
        )
        check(
            "save/load full S entries",
            np.allclose(dd_all.overlap_entries, dd_reloaded.overlap_entries),
        )
        check(
            "save/load full H_init entries",
            np.allclose(
                dd_all.initial_hamiltonian_entries,
        ...
        check(
            "info.json orbits_quantity == 90",
            info_json["orbits_quantity"] == 90,
            f"got {info_json['orbits_quantity']}",
        )
        check(
            "info.json occupation > 0",
            info_json["occupation"] > 0,
        ...
        check(
            "S shells >= 2", len(eom.get("S", [])) >= 2, f"S shells: {eom.get('S', [])}"
        )
        shutil.rmtree(tmp_dir2, ignore_errors=True)

        # ── 9. from_aimspy with template ─────────────────────────────
        dd_templ = DeepHData.from_directory(DEEPH_DIR)
        dd_from_templ = DeepHData.from_aimspy(
            s, hamiltonian=H_aimspy, template=dd_templ
        )
        check(
            "from_aimspy(template) n_atoms", dd_from_templ.n_atoms == dd_templ.n_atoms
        )
        check(
            "from_aimspy(template) atom_symbols match",
            list(dd_from_templ.atom_symbols) == list(dd_templ.atom_symbols),
        )
        check(
            "from_aimspy(template) elements_orbital_map match",
            dd_from_templ.elements_orbital_map == dd_templ.elements_orbital_map,
        )
        check(
            "from_aimspy(template) H entries shape matches",
            dd_from_templ.entries.shape[0] == dd_all.entries.shape[0],
        )
        check(
            "from_aimspy(template) n_basis from template",
