...
def _run_capture(work_dir: Path, logfile: str):
    ...
    try:
        ...
        if rank == 0:
            return calc.first_order_hamiltonian, calc.hamiltonian, calc.structure
...
if rank == 0:
    ...
    all_ok &= _ok("full-memory CPSCF converged", _cpscf_converged(FULL_DIR, FULL_LOG))
...
if rank == 0:
    ...
    if isinstance(fo_serial, list) and len(fo_serial) == 3:
        ...
        all_ok &= _ok(
            ...
            all(mx is not None and mx.n_pairs == H_serial.n_pairs for mx in fo_serial),
        )
    all_ok &= _ok("serial CPSCF converged", _cpscf_converged(SERIAL_DIR, SERIAL_LOG))
    ...
    if (
        ...
        all_ok &= _ok(
            "serial vs full comparison runnable", False, "missing fo_full or fo_serial"
        )
