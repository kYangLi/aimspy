...
class AimspyInfo:
    ...
    basisfn_species: np.ndarray = field(
...
def _aimspy_info_from_c(c):
    ...
    if c.frac_coords_ptr:
        # frac_coords is dimensionless (inv_lattice @ cart_coords), NOT in Bohr
        frac = _view_f(c.frac_coords_ptr, (3, N)).T.copy()
    else:
        frac = None
    ...
    return AimspyInfo(
        ...
        flag_rel=int(c.flag_rel),
        spin_treatment=int(c.spin_treatment),
        myid=int(c.myid),
        n_tasks=int(c.n_tasks),
        output_level=ol_str,
        ...
        basisfn_n=basisfn_n,
        basisfn_l=basisfn_l,
        basisfn_type=basisfn_type,
        basisfn_species=basisfn_species,
    )
